...
if debugX then
	warn('Initialising Nexium')
end

local scriptRelease
if getgenv and getgenv().scriptRelease ~= nil then
    scriptRelease = getgenv().scriptRelease
else
    scriptRelease = _G.scriptRelease
end

if scriptRelease == nil then
	scriptRelease = "Unknown"
end
...
local NexiumFolder = "Nexium"
local ConfigurationFolder = NexiumFolder.."/Configurations"
local ConfigurationExtension = ".nxm"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		nexiumOpen = {Type = 'bind', Value = 'K', Name = 'Nexium Keybind'},
		-- buildwarnings
		-- Nexiumprompts
	},
	System = {
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(NexiumFolder) then
				if isfile and isfile(NexiumFolder..'/settings'..ConfigurationExtension) then
					file = readfile(NexiumFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"nexiumOpen":{"Value":"K","Type":"bind","Name":"Nexium Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Nexium Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Nexium had an issue accessing configuration saving capability.')
		end
	end
end
...
local NexiumLibrary = {
	...
	Interface = nil,
	ExecutionId = (function() local t={"n","m"} for i=1,6 do table.insert(t,string.char(math.random(97,122))) end for i=#t,2,-1 do local j=math.random(i) t[i],t[j]=t[j],t[i] end return table.concat(t) end)(),
}

local executionId
if getgenv and getgenv().executionId ~= nil then
    executionId = getgenv().executionId
else
    executionId = _G.executionId
end

if executionId ~= nil then
	warn('Nexium | Interface already loaded.')
	return false
else
	_G.executionId = NexiumLibrary.ExecutionId
	if getgenv then
		getgenv().executionId = NexiumLibrary.ExecutionId
	end
end
...
local Nexium = useStudio and script.Parent:FindFirstChild('Nexium') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if Nexium:FindFirstChild('Build') and Nexium.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Nexium | Build Mismatch')
		print('Nexium may encounter issues as you are running an incompatible interface version ('.. ((Nexium:FindFirstChild('Build') and Nexium.Build.Value) or 'No Build') ..').\n\nThis version of Nexium is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Nexium = Nexium, useStudio and script.Parent:FindFirstChild('Nexium') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

Nexium.Enabled = false

if gethui then
	Nexium.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(Nexium)
	Nexium.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Nexium.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Nexium.Name and Interface ~= Nexium then
			Interface.Enabled = false
			Interface.Name = "Nexium-Old"
		end
	end
elseif not useStudio then
...
if Nexium.AbsoluteSize.X < minSize.X and Nexium.AbsoluteSize.Y < minSize.Y then
...
if NexiumLibrary and NexiumLibrary.Interface == nil then
	NexiumLibrary.Interface = Nexium
end

-- Object Variables

local Main = Nexium.Main
local MPrompt = Nexium:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Nexium:FindFirstChild('Drag')
...
LoadingFrame.Version.Text = scriptRelease

Nexium.DisplayOrder = 100

local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/Nexium/refs/heads/main/icons.lua')
...
local Notifications = Nexium.Notifications

local SelectedTheme = NexiumLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = NexiumLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Nexium.Main.BackgroundColor3 = SelectedTheme.Background
	Nexium.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Nexium.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Nexium.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Nexium.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Nexium.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Nexium.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Nexium.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Nexium.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Nexium:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Nexium | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Nexium | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Nexium had an issue decoding the configuration file, please try delete the file and reopen Nexium.') return end

	for FlagName, Flag in pairs(NexiumLibrary.Flags) do
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(NexiumLibrary.Flags) do
...
function NexiumLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			NexiumLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Nexium'.", Duration = 7, Image = 4400697855})
		else
			NexiumLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.nexiumOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(NexiumFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Nexium Settings', 0, true)

	if TabList['Nexium Settings'] then
		TabList['Nexium Settings'].LayoutOrder = 1000
	end

	if Elements['Nexium Settings'] then
		Elements['Nexium Settings'].LayoutOrder = 1000
	end
...
function NexiumLibrary:CreateWindow(Settings)
	if Nexium:FindFirstChild('Loading') then
		if getgenv and not getgenv().NexiumCached then
			Nexium.Enabled = true
		end
	end

	if getgenv then getgenv().NexiumCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				NexiumLibrary:Notify({Title = 'Build Mismatch', Content = 'Nexium may encounter issues as you are running an incompatible interface version ('.. ((Nexium:FindFirstChild('Build') and Nexium.Build.Value) or 'No Build') ..').\n\nThis version of Nexium is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(NexiumFolder) then
		makefolder(NexiumFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Nexium"
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(NexiumFolder.."/Discord Invites") then
			makefolder(NexiumFolder.."/Discord Invites")
		end

		if isfile and not isfile(NexiumFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(NexiumFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Nexium RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(NexiumFolder.."/Key System") then
			makefolder(NexiumFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Nexium | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile and isfile(NexiumFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(NexiumFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(NexiumFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						NexiumLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Nexium | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= NexiumLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					NexiumLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Nexium.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Nexium | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					NexiumLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Nexium | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					NexiumLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Nexium | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					NexiumLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= NexiumLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Nexium | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Transparency = 0}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						NexiumLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Nexium | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					NexiumLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			NexiumLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			NexiumLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Nexium had an issue creating settings.') end
...
function NexiumLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function NexiumLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function NexiumLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Nexium Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.nexiumOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function NexiumLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				NexiumLibrary:Notify({Title = "Nexium Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			NexiumLibrary:Notify({Title = "Nexium Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Nexium Configurations Error | '..tostring(result))
			NexiumLibrary:Notify({Title = "Nexium Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = NexiumLibrary:CreateWindow({
		Name = "Nexium Example Window",
		LoadingTitle = "Nexium Interface Suite",
		Theme = 'Default',
		Icon = 0,
		LoadingSubtitle = "by Nexium",
		ConfigurationSaving = {
			Enabled = true,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using Nexium may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Nexium to get the key from
	...
	--NexiumLibrary:Notify({Title = "Nexium Interface", Content = "Welcome to Nexium. These - are the brand new notification design for Nexium, with custom sizing and Nexium calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(NexiumLibrary.Theme) do
...
task.delay(4, function()
	NexiumLibrary.LoadConfiguration()
